...
class BatchedCGSparse(Benchmark):
    ...
    def setup(self, n, batch_size, dense_solve):
        ...
        self.batched = "nd_support" in inspect.signature(make_system).parameters
        if self.batched:
        ...
        if dense_solve:
            P = P_sparse.toarray()
            if self.batched or batch_size == 1:
                self.P = P
            else:
                # simulate many different LHS
                self.P = np.stack([P] * batch_size)
            self.b = rng.standard_normal((batch_size, n*n, 1))
        else:
            self.P = P_sparse
    ...
    def solve(self, n, batch_size, dense_solve):
        if dense_solve:
            assert self.b.shape == (batch_size, n*n, 1)
            linalg.solve(self.P, self.b, assume_a="positive definite")
        elif self.batched:
            assert self.b.shape == (batch_size, n*n) if batch_size > 1 else (n*n,)
            cg(self.P, self.b)
        else:
...
class BatchedCGDense(XPBenchmark):
    param_names = (*XPBenchmark.param_names, '(n,n)', 'batch_size', 'dense_solve')
    params = (
        *XPBenchmark.params,
        [2, 12],
        [1, 100, 500, 1000],
    ...
    def setup(self, backend, n, batch_size, dense_solve):
        ...
        self.batched = "nd_support" in inspect.signature(make_system).parameters
        if self.batched or dense_solve:
            if batch_size > 1:
            ...
            self.A = [_create_dense_random(n) for _ in range(batch_size)]
            self.b = [xp.asarray(rng.standard_normal(n*n)) for _ in range(batch_size)]

        if dense_solve:
            identity = xp.broadcast_to(xp.eye(self.A.shape[-1]), self.A.shape)
    ...
    def time_solve(self, backend, n, batch_size, dense_solve):
        if dense_solve:
            linalg.solve(self.A, self.b, assume_a="positive definite")
        elif self.batched:
            cg(self.A, self.b)
        else:
            for i in range(batch_size):
                cg(self.A[i], self.b[i])
...
def _create_dense_random(n, batch_shape=None, xp=None):
    ...
    reg = 1e-3
    if batch_shape:
        reg = reg * rng.uniform(0.5, 1.5, size=(*batch_shape, 1))
        M = xp.broadcast_to(M[np.newaxis, ...], (*batch_shape, n*n, n*n))
    def matvec(x):
        return xp.squeeze(M.mT @ (M @ x[..., xp.newaxis]), axis=-1) + reg * x

    if "nd_support" in inspect.signature(make_system).parameters:
