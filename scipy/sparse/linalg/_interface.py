...
class LinearOperator:
    ...
    def _matmat(self, X):
        ...
        return xp.concat(
            [self.matvec(X[..., :, i, xp.newaxis]) for i in range(X.shape[-1])],
            axis=-1
        )
    ...
    def _rmatvec(self, x):
        ...
        if type(self)._adjoint == LinearOperator._adjoint:
            ...
            if (hasattr(self, "_rmatmat")
                ...
                if x.shape[-1] == M:
                    return xp.reshape(self._rmatmat(x[..., xp.newaxis]), *x.shape)
                else:
    ...
    def _rmatmat(self, X):
        ...
        if type(self)._adjoint == LinearOperator._adjoint:
            ...
            return xp.concat(
                [self.rmatvec(X[..., :, i, xp.newaxis]) for i in range(X.shape[-1])],
                axis=-1
            )
        else:
