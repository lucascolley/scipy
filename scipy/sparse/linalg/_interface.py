...
class _ScaledLinearOperator(LinearOperator):
    ...
    def _conj_alpha(self):
        xp = self._xp
        xp_alpha = xp.asarray(self.args[1])
        if xp.isdtype(xp_alpha.dtype, "complex floating"):
            return complex(xp.conj(xp_alpha))
        else:
            return self.args[1]
    ...
    def _rmatvec(self, x):
        return self._conj_alpha() * self.args[0].rmatvec(x)

    def _rmatmat(self, x):
        return self._conj_alpha() * self.args[0].rmatmat(x)
...
def _xp_aslinearoperator(A):
    ...
    if hasattr(A, "shape") and hasattr(A, "matvec"):
        ...
        return LinearOperator(
            A.shape, A.matvec, rmatvec=rmatvec, rmatmat=rmatmat, dtype=dtype, xp=xp
        ), xp
